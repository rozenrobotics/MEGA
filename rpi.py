...
# Initialize the RobotAPI object for controlling the robot
...
xm, ym = cropbox[1][0] + (cropbox[1][1] - cropbox[1][0]) // 2, cropbox[0][0] + (cropbox[0][1] - cropbox[0][0]) // 2
...
def gates(sensor, low, high, lim, graphics=0):
    ...
    if cntcnts >= 2:
        ...
        length = abs(x0 - x1)
        gp = (cropbox[1][0] + (x0 + x1) // 2, cropbox[0][0] + (((y0 + y1) // 2) + ((h0 + h1) // 2) // 2))

    elif cntcnts == 1:
        x1, y1, w1, h1 = cv.boundingRect(cm1)
        x1 = x1 + w1 // 2

        if x1 < 320: gp = (cropbox[1][1], ym)
        else: gp = (cropbox[1][0], ym)
    ...
    if graphics:
        ...
        if cntcnts > 1:
            cv.circle(frame, (cropbox[1][0] + x0, cropbox[0][0] + y0 + h0 // 2), 3, green, -1)
            cv.circle(frame, (cropbox[1][0] + x1, cropbox[0][0] + y1 + h1 // 2), 3, green, -1)
...
def ball(sensor, low, high, lim, graphics=0):
    ...
    if cntcnts > 0:
        ...
        x0, y0, w0, h0 = cv.boundingRect(cm0)
        conrect = (w0, h0)
        gp, dist = (cropbox[1][0] + x0 + w0 // 2, cropbox[0][0] + y0 + h0 // 2), cropbox[0][1] - (cropbox[0][0] + y0 + h0)
...
def marker(sensor, low, high, graphics=0):
    ...
    for c in cv.findContours(mask, cv.RETR_CCOMP, cv.CHAIN_APPROX_SIMPLE)[0]:
        sc = cv.contourArea(c)
        if sc > sm0 and sc > lim: cntcnts, cm0, sm0 = cntcnts + 1, c, sc
    ...
    if graphics:
        ...
        if cntcnts > 0: cv.rectangle(frame, (x0, y0), (x0 + w0, y0 + h0), green, 2)
        # cv.circle(frame, (gp[0], gp[1]), 5, green, -1)
...
def telemetry(image):
    ...
    # goal point
    if maincam: cv.arrowedLine(image, (cropbox[1][1] // 2, cropbox[0][1]), (gp [0], gp [1]), pink, 2, tipLength=0.025)
    else:       cv.arrowedLine(image, (cropbox[1][1] // 2, cropbox[0][1]), (dgp[0], dgp[1]), pink, 2, tipLength=0.025)

    # compass
    strcompass = ('', 0)
    if azimuth > 99:  strcompass = (str(azimuth), 300)
    elif azimuth > 9: strcompass = (str(azimuth), 308)
    else:             strcompass = (str(azimuth), 314)
    ...
    if tim > timfps + 1: fps, cntfps, timfps = cntfps, 0, tim
    ...
    cv.putText(image, ("X: " + str(gp[0]) + "   Y: " + str(gp[1])), (20, 20), font, 1, black, 1)
...
def hsvcolor():
    ...
    frame = cv.bitwise_or(frame, frame, mask=cv.inRange(cv.cvtColor(frame, cv.COLOR_BGR2HSV), color[0], color[1]))


def xy2angle(x, y):
    angle, x, y = 0, x - 320, y - 240

    if   y >  0 <= x: angle =       np.arctan(float(abs(x) / abs(y))) * 180 / math.pi if abs(x) <= abs(y) else 90  - np.arctan(float(abs(y) / abs(x))) * 180 / math.pi
    elif y <= 0 <  x: angle = 90  + np.arctan(float(abs(y) / abs(x))) * 180 / math.pi if abs(y) <= abs(x) else 180 - np.arctan(float(abs(x) / abs(y))) * 180 / math.pi
    elif y <  0 >= x: angle = 180 + np.arctan(float(abs(x) / abs(y))) * 180 / math.pi if abs(x) <= abs(y) else 270 - np.arctan(float(abs(y) / abs(x))) * 180 / math.pi
    elif y >= 0 >  x: angle = 270 + np.arctan(float(abs(y) / abs(x))) * 180 / math.pi if abs(y) <= abs(x) else 360 - np.arctan(float(abs(x) / abs(y))) * 180 / math.pi
...
def arrow(lim0, lim1, graphics=0):
    ...
    for c in cntswhite:
        ...
        if sc > lim0 and sc > 0.7 * h0 * w0:
            ...
            cntsblack = cv.findContours(maskblack, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE)[0]
    ...
    if cntcnts > 0:
        conrect = (w, h)
        gp, dist = (cropbox[1][0] + x + w // 2, cropbox[0][0] + y + h // 2), cropbox[0][1] - (cropbox[0][0] + y + h)
...
if __name__ == "__main__":
    ...
    for i in range(30): frame = rapi.get_frame(wait_new_frame=1)
    while True:
        ...
        modego, gp, dgp, goal, speed, sensor = 0, (xm, ym), (320, 240), 0, 0, frame[cropbox[0][0]:cropbox[0][1], cropbox[1][0]:cropbox[1][1]]
        dist = 1000
        flnocnt = 0

        if state == 'hsv': hsvcolor()
        elif mode == 2:
            ...
            # align to marker by distance first wave
                ...
                if fllocal == 0:
                    ...
                    if   error < -100: goal = 90
                    elif error >  100: goal = 270
                    else: goal = 0
            ...
            # return gates
                ...
                if fllocal == 0:
                    ...
                    if flnocnt: goal = pd_compass(90)

                    if length > 400 and square > 4000: tim, fllocal = time.time(), 2
                        # fllocal = 1
                        # tim = time.time()
            ...
            # go forward before orange circle
                ...
                if fllocal == 0:
                    ...
                    marker(cv.cvtColor(frame, cv.COLOR_BGR2HSV), hsvorange[0], hsvorange[1], 1)
            ...
            # align to down marker and bombing
                ...
                modego, goal, speed, megacompass = 1, angle, 10 if dist < 100 else 30, compass[1]

                if speed >  30: speed = 30
                if speed < -30: speed = -30

                if dist < 50: goal, speed = 0, 5
            ...
            # first ball
                ...
                if tim + protectballs < time.time(): state = 8
            ...
            # parking
                ...
                if fllocal == 0:
                    ...
                    if   errorx < -40: goal = 90
                    elif errorx >  40: goal = 270
