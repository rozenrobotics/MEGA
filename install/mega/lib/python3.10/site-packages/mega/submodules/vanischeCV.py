...
class Rect:
    ...
    def with_offset(self, offset: int) -> Rect:
        return Rect(
            self.x - offset // 2, self.y - offset // 2, self.w + offset, self.h + offset
        )

    def with_roi_offset(self, roi: ROI) -> Rect:
        return Rect(self.x + roi.x1, self.y + roi.y1, self.w, self.h)
...
class ROI:
    ...
    def from_rect(rect: Rect) -> ROI:
        return ROI(y1=rect.y, y2=rect.y + rect.h, x1=rect.x, x2=rect.x + rect.w)
...
class Frame:
    ...
    def hsv_tb_mask(
        self, src: Frame, winname: str = "hsv trackbars", mask_color: Color = None
    ) -> Frame:
    ...
    def create_thresh_tb_window(
        winname: str = "thresh trackbar", default_value: int = 0
    ) -> None:
    ...
    def draw_rect(
        self, rect: Rect, color: Color = Colors.PURPLE, thickness: int = 1
    ) -> Frame:
    ...
    def draw_cont_rect(
        self, cont: Contour, color: Color = Colors.PURPLE, thickness: int = 1
    ) -> Frame:

        cv2.rectangle(
            self.src,
            (cont.x, cont.y),
            (cont.x + cont.w, cont.y + cont.h),
            color,
            thickness,
        )
...
class Contour:
    ...
    def approx(self, value: float = 0.01) -> Contour:
        self.cont = cv2.approxPolyDP(
            self.cont, value * cv2.arcLength(self.cont, True), True
        )
        return self
