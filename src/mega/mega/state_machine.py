...
class StateMachine(Node):
    def gates(self) -> None:
        if not self.new_frames:
            return

        _gates_mask: Frame = Frame(self.scl_hsv, "hsv").in_range(Bounds.YELLOW_GATES)
        _gates: Conts = _gates_mask.find_biggest_conts(conts_num=2)
    ...
    def __init__(self) -> None:
        ...
        self.state_tmr = time()
        self.new_frames: bool

        self.scs_src = MatLike
        self.scl_bgr = MatLike
        self.scl_hsv = MatLike
        self.scl_mon = MatLike
        self.scr_bgr = MatLike
        self.scr_hsv = MatLike
        self.scr_mon = MatLike
        self.sc_depth_map = MatLike
        ...
        self.pubs: dict = {
            "panno": self.create_publisher(Image, "/mega/panno", 10),
